            ...
            <tbody>
                ...
                @foreach($table_ensemble as $yop)
                    <tr id="ensemble-id-{{ $yop['id'] }}">
                        ...
                        <td>{{Form::text('en_budget_commande', $yop['en_budget_commande'], ["class" => "form-tab width-input-number sum-ensemble-cmd"])}}</td>
                        <td>{{Form::text('be_val_1', $yop['CDP'], ["class" => "form-tab tab-number sum-ensemble-heure"])}}</td>
                        <td>{{Form::text('be_val_2', $yop['TEC'], ["class" => "form-tab tab-number sum-ensemble-heure"])}}</td>
                        <td>{{Form::text('be_val_3', $yop['MET'], ["class" => "form-tab tab-number sum-ensemble-heure"])}}</td>
                        <td>{{Form::text('be_val_4', $yop['MAINT'], ["class" => "form-tab tab-number sum-ensemble-heure"])}}</td>
                        <td>{{Form::text('be_val_5', $yop['OPE'], ["class" => "form-tab tab-number sum-ensemble-heure"])}}</td>
                        <td>{{Form::text('be_val_6', $yop['DIV'], ["class" => "form-tab tab-number sum-ensemble-heure"])}}</td>
